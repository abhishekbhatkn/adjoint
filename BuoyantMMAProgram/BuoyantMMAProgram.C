...
private:
    ...
    // member variables
    	Foam::volScalarField eta_MMA, eta_old, dfdeta;
    Foam::scalar penalty, volumeConstraint, designVolume, optEpsilon, porosity_s, porosity_f, factorT, factorP, refRho, Cp;
    Foam::label nOptSteps, maxPiggyLoop, maxoutit, maxInnerLoop, designSize;
	 ...
	 dfdeta("dfdeta",eta)
{
    ...
    forAll(designSpaceCells,i){
	    ...
	    eta[j] = 1.0; //volumeConstraint; //*designVolume/(mesh.V()[j]*designSpaceCells.size());
    }
    ...
    scalar calcCost(){
        scalar J = 0;
	J = -factorT*(calcTempCost())/(200) + factorP*(calcPrCost() -(1.0e-08))/((1.3e-06)-(1.0e-08));
	return J;
    }
